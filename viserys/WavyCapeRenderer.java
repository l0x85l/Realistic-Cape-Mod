...
public class WavyCapeRenderer {
    ...
    public void onRenderPlayer(final EventRenderPlayer e) {
        ...
        partialTickTest = e.getPartialRenderTick();
        ...
        if (check) {
            ...
            for (final List<Node> nodes2 : nodes) for (final Node node : nodes2) { node.aY -= gravity / 2; node.update(pX, pY, pZ, player); }
            ...
            final ResourceLocation capeTex = new ResourceLocation("cape/cape.png"); // 1024 * 768
            ...
            mc.renderEngine.bindTexture(capeTex);
            GL11.glPopMatrix();
            for (int i = 0; i < nodes.size(); i++) for (int j = 0; j < horzNodes; j++) {
                final Node node = nodes.get(i).get(j);
